...
void SettingsState::initFonts()
{
	...
	{
		throw("ERROR::MAINMENUSTATE: FAILED TO LOAD FONT ::PokemonXandY.ttf");
	}
	if (!this->fontSpe.loadFromFile("Resources/Fonts/LouisGeorgeCafe.ttf"))
	{
		throw("ERROR::MAINMENUSTATE: FAILED TO LOAD FONT ::LouisGeorgeCafe.ttf");
	}
}
...
{
	this->texts["TITLE"] = new gui::Text(0.f, 30.f, static_cast<float>(this->stateData->window->getSize().x), 100.f,
	...
	this->buttons["APPLY"] = new gui::Button(static_cast<float>(this->stateData->window->getSize().x) - 200.f, static_cast<float>(this->stateData->window->getSize().y) - 112.5f, 150.f, 62.5f,
		&this->font, "Save", 50,
		sf::Color(255, 255, 255, 200), sf::Color(255, 255, 255, 255), sf::Color(20, 20, 20, 200),
	...
	// EEG
	this->buttons["EEG"] = new gui::Button(static_cast<float>(this->stateData->window->getSize().x), 0, 150.f, 62.5f,
		&this->font, "TEST", 50,
...
{
	if (!this->buffer.loadFromFile("Resources/Audio/Aaargh.wav"))
		throw("ERROR::SETTINGSSTATE: FAILED TO LOAD SOUNDBUFFER");
}
// #### Init Functions #### //
...
void SettingsState::updateGui(const float& dt)
{
	...
	if (this->buttons["EEG"]->isPressed())	// Easter egg sound button
	{
		this->sound.setBuffer(this->buffer);
		if (this->sound.getStatus() != sf::SoundSource::Playing)
			this->sound.play();
	}
